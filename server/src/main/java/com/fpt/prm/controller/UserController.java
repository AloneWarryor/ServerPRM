...
public class UserController {
		...
		public ResponseEntity<BaseResponse> addArticle(@RequestBody RegMappingEntity UserInfo) {
			BaseResponse baseResponse = new BaseResponse(0, null);
			try {
				int status = accountMapper.regUser(new Account(UserInfo.getUsername(), UserInfo.getPassword(), UserInfo.getFullName(), UserInfo.getAddress(), UserInfo.getPhoneNumber(), UserInfo.getEmail(), null, "0", "user", UserInfo.getUsername(), new Date().getTime(), UserInfo.getUsername(), new Date().getTime()));
